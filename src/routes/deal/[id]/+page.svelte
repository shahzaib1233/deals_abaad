...
<div class="">
	{#if data.category && !data.category.comingSoon}
		<Hero
		...
		<div class="my-container mt-[6.25rem]">
			...
			<div class="flex flex-wrap gap-[1.5rem]">
				<!-- {data.data.DealCategory.map((item: any, index: number) => (
	          <ProductCard
	            key={index}
	            ...
	            discount={`${item.deal.saleper}% OFF`}
	            rating={4.2}
	            img={`${process.env.NEXT_PUBLIC_BASE_URL}img/${item.deal.frontImage}`}
	          />
	        ))} -->
				{#each data.category.DealCategory as item}
					<ProductCard
						...
						discount={`${item.deal.saleper}% OFF`}
						rating={4.2}
						img={`${import.meta.env.VITE_BASE_URL}deal/img/${item.deal.frontImage}`}
					/>
				{/each}
			</div>
		</div>
	{:else}
		<div class="my-container mt-[6.25rem] text-center py-12">
			<p class="text-2xl font-bold mb-4">Coming Soon</p>
			<p class="text-gray-600">Category details will be displayed here once the backend is integrated.</p>
		</div>
	{/if}
</div>
